...
def apply_logistic_regression(x_train, y_train, x_val, y_val, version):
    ...
    if version == 'naive': 
        ...
        p_val = clf.predict(x_val).flatten()
        theta = clf.theta
        output_path = output_path_naive
        plot_path = plot_path_naive
    else:
        x_train_adjusted, y_train_adjusted = upsample_minority_class(x_train, y_train)
...
def calculate_accuracies(p_val, y_val):
    ...
    if (p_val.shape != y_val.shape):
        kappa_val = np.sum(y_val == 1) / np.sum(y_val == 0)
        y_val_1 = np.tile(y_val[y_val == 1],int(1/kappa_val))
        y_val = np.concatenate([y_val[y_val == 0],y_val_1])
    # *** START CODE HERE ***
    true_pos = np.sum((p_val == 1) & (y_val == 1))
    true_neg = np.sum((p_val == 0) & (y_val == 0))
    false_pos = np.sum((p_val == 1) & (y_val == 0))
    false_neg = np.sum((p_val == 0) & (y_val == 1))
...
def upsample_minority_class(x_train, y_train):
    ...
    # *** START CODE HERE ***
    kappa_train = np.sum(y_train == 1) / np.sum(y_train == 0)
    x_train_1 = np.tile(x_train[y_train == 1],(int(1/kappa_train),1))
    x_train_new = np.concatenate([x_train[y_train == 0],x_train_1])
    y_train_1 = np.tile(y_train[y_train == 1],int(1/kappa_train))
    y_train_new = np.concatenate([y_train[y_train == 0],y_train_1])
    # *** END CODE HERE
