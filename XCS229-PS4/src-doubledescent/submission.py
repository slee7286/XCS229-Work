...
def regression(train_path, validation_path):
    ...
    val_err = (1 / (2 * x_validation.shape[0])) * np.dot(pred - y_validation, pred - y_validation)
    # *** END CODE HERE
...
def ridge_regression(train_path, validation_path):
    ...
    n = x_train.shape[1]
    for lam in reg_list:
        beta = np.linalg.pinv(x_train.T @ x_train + lam * np.eye(n)) @ x_train.T @ y_train
        pred = x_validation @ beta
        error = (1 / (2 * x_validation.shape[0])) * np.dot(pred - y_validation, pred - y_validation)
