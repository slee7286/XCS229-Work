...
def softmax(x):
    ...
    # *** START CODE HERE ***
    exp_x = np.exp(x)
    class_sum = np.sum(exp_x,axis=1,keepdims=True)
    return exp_x / class_sum
    # *** END CODE HERE ***
...
def sigmoid(x):
    ...
    result[nonneg_mask] = 1 / (1 + np.exp(-x[nonneg_mask]))
...
def forward_prop(data, labels, params):
    ...
    """
    # *** START CODE HERE ***
    layer_1_1 = data @ params["W1"] + params["b1"]
    layer_1_2 = sigmoid(layer_1_1)
    layer_2_1 = layer_1_2 @ params["W2"] + params["b2"]
    output = softmax(layer_2_1)
    average_loss = -np.sum(labels * np.log(output)) / output.shape[0]
    return (layer_1_2,output,average_loss)    
    # *** END CODE HERE ***
...
def backward_prop(data, labels, params, forward_prop_func):
    ...
    """
    # *** START CODE HERE ***
    a1, output, average_loss = forward_prop_func(data,labels,params)
    dJdz2 = (output - labels) / output.shape[0]
    dJdW2 = a1.T @ dJdz2
    dJdb2 = np.sum(dJdz2, axis=0)
    dJda1 = dJdz2 @ params["W2"].T
    dJdz1 = dJda1 * (a1 * (1-a1))
    dJdW1 = data.T @ dJdz1
    dJdb1 = np.sum(dJdz1,axis=0)
    return {"W1":dJdW1,"W2":dJdW2,"b1":dJdb1,"b2":dJdb2}
    # *** END CODE HERE ***
...
def gradient_descent_epoch(train_data, train_labels, learning_rate, batch_size, params, forward_prop_func, backward_prop_func):
    ...
    for index in range(0, train_data.shape[0], batch_size):
        ...
        backward_prop = backward_prop_func(batch_data,batch_labels,params,forward_prop_func)
        for key in params:
            params[key] -= learning_rate * backward_prop[key]
    # *** END CODE HERE ***
...
def run_train_test(name, all_data, all_labels, backward_prop_func, num_epochs, plot=True, test_set = False):
    params, cost_train, cost_dev, accuracy_train, accuracy_dev = nn_train(
        ...
        num_hidden=HPARAMS['num_hidden'], learning_rate=HPARAMS['learning_rate'],
        num_epochs=num_epochs, batch_size=HPARAMS['batch_size']
    )
